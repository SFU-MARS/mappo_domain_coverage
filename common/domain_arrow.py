...
class Arrow(object):
    def __init__(self, center=np.array([0, 0]), edge_length=8):
        ...
        self.A = np.array([center[0] + edge_length / 2, center[1] - ((1 / math.sqrt(3)) * (edge_length / 2))])
        self.B = np.array([center[0], center[1] + (self.height - ((1 / math.sqrt(3)) * (edge_length / 2)))])
        self.C = np.array([center[0] - edge_length / 2, center[1] - ((1 / math.sqrt(3)) * (edge_length / 2))])
        self.D = np.array([center[0], (center[1] - ((1 / math.sqrt(3)) * (edge_length / 2))) + self.height / 3])

        self.edge_AB = LineString([self.A, self.B])
        self.edge_BC = LineString([self.B, self.C])
        self.edge_CD = LineString([self.C, self.D])
        self.edge_DA = LineString([self.D, self.A])
    ...
    def measure_distance(self, point):
        ...
        if self.shape.contains(point):
            ...
            c3, _ = nearest_points(self.edge_CD, point)
            c4, _ = nearest_points(self.edge_DA, point)
            projection_point = [np.array([c1.x, c1.y]), np.array([c2.x, c2.y]),
                                np.array([c3.x, c3.y]), np.array([c4.x, c4.y])]
